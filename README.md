...
Bypass Charge can be enabled, disabled, start and stop battery level can be set via sysfs.

n most systems this will be mounted at `/sys/class/power_supply/ayaneo:bypass_charge/` and provides the following files:

#### charge_type

Read/write.

Enables or disables the bypass charge. Accepts a value of 6 and 0 or "Custom" and "Unknown".

|Value|Description|
|-|-|
|6 or "Custom"|Enables the bypass charge. Battery charging will depend on the start and stop values.|
|0 or "Unknown"|Disables the bypass charge. The battery is normally charged.|
...
New values can be set as follows:
```shell
$ echo "0" | sudo tee /sys/class/power_supply/ayaneo:bypass_charge/charge_type
Unknown
